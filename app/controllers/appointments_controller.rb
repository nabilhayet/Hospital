class AppointmentsController < ApplicationController
  ...
  post '/appointments' do
    ...
    @appointment = Appointment.all.select do |apt|
      (apt.time.strftime("%I:%M") == params[:time] && apt.date.to_s == params[:date]) && (apt.patient.id == @patient.id || apt.doctor.id == params[:patient][:doctor_ids])
    end
      if @patient
  ...
  patch '/appointments/:id' do
    @patient = ApplicationController.current_user(session)
    @apt = Appointment.find_by_id(params[:id])
    @appointment = Appointment.select{|apt| apt.time.strftime("%I:%M")==params[:time] && apt.date.to_s==params[:date] && (apt.patient.id==@patient.id || apt.doctor.id==params[:doctor_id])}
 ...
 get '/update/doctor' do
  ...
  patch '/appointmentss/:id' do
    ...
    @apt = Appointment.find_by_id(params[:id])
    @patient = @apt.patient
    @appointment = Appointment.select{|apt| apt.time.strftime("%I:%M")==params[:time] && apt.date.to_s==params[:date] && (apt.patient.id==@patient.id || apt.doctor.id==@doctor.id)}
